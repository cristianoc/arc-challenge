...
class Grid:
    ...
    def rotate(self, direction: Direction) -> 'Grid':
        if direction == Direction.CLOCKWISE:
            ...
            rotated_grid = self._rotate_grid(self._rotate_grid(self._rotate_grid(self.raw)))
    ...
    def translate(self, dx: int, dy: int) -> 'Grid':
        new_grid: Raw = [[BLACK] * len(self.raw[0]) for _ in range(len(self.raw))]
    ...
    def color_change(self, from_color: Color, to_color: Color) -> 'Grid':
        new_grid = [[to_color if cell == from_color else cell for cell in row] for row in self.raw]
        return Grid(new_grid)
    ...
    def map(self, func: Callable[[int, int], 'Grid']) -> 'Grid':
        ...
        size = self.size()
        new_grid: List[List[Raw]] = [[func(i, j).raw for j in range(size)] for i in range(size)]
    ...
    def display(self, title: Optional[str] = None) -> None:
        ...
        for i in range(len(data)):
            for j in range(len(data[0])):
                rect = plt.Rectangle((j - 0.5, i - 0.5), 1, 1, edgecolor='grey', facecolor='none', linewidth=1)
                ax.add_patch(rect)
