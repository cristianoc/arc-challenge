...
def dominant_frequency_and_period(y: npt.NDArray[np.int_], s: str) -> None:
    ...
    frequencies_large = np.fft.fftfreq(
        len(y), d=1)  # d=1 means sample spacing is 1
    ...
    print(f"Dominate Frequency: {dominant_frequency_large}")
    print(f"Dominate Period: {dominant_period_large}")
