...
def visualize_transformations_with_labels(grid, transformations_by_level):
    # Determine the maximum number of non-empty transformations per level
    max_cols = max(sum(1 for t in level if get_transformation_label(t)) for level in transformations_by_level)
    rows = len(transformations_by_level)

    fig, axes = plt.subplots(rows, max_cols, figsize=(max_cols * 1.5, rows * 1.5))

    # If there's only one row, axes might not be an array, so we handle that
    if rows == 1:
        axes = np.array([axes])
    
    for i, level in enumerate(transformations_by_level):
        col_index = 0  # Track the column position for non-empty grids
        for j, transformation_sequence in enumerate(level):
            label = get_transformation_label(transformation_sequence)
            if label:  # Only process non-empty grids
                transformed_grid = apply_transformations(grid.copy(), transformation_sequence)
                ax = axes[i, col_index] if rows > 1 else axes[col_index]
        ...
        for k in range(col_index, max_cols):
...
# Increase size to 128x128 pixels for better resolution
...
# The normal form is defined as a tuple of optional transformations
...
# Simplify the normal form by removing None elements and counting the length
...
def generate_transformations_by_level():
    ...
    for length, form in simplified_forms:
        transformations_by_level[length].append(form)

    return transformations_by_level


# Example transformation sequences by level generated by computation
