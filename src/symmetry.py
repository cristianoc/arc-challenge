...
# Brute-force translation search with expanding offset distances using iterator and an unknown value
def find_matching_subgrid_offset(
    ...
    for _ in range(
        (2 * max_distance + 1) ** 2
    ):  # Iterate until all relevant offsets within the LxL limit are tested
        ...
        if check_visible_subgrid_with_unknown(g1, g2, ox, oy, unknown):
            return (ox, oy)  # Return the translation if a match is found

    return None  # Return None if no valid translation is found
