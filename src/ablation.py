...
# Function to process tasks for a given difficulty level (moved outside of ablation_study)
...
def ablation_study():
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        futures = [executor.submit(evaluate_difficulty, difficulty_level) for difficulty_level in range(1, 13)]
    ...
    logger.error(f"{get_current_time()} - Ablation study completed. Results saved to 'ablation_results.json'.")
