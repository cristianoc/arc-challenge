...
def ablation_study() -> None:
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        futures = [
            executor.submit(evaluate_difficulty, difficulty_level)
    ...
    evaluation_accuracies = [results[level]["evaluation_data"] for level in levels]

    # Plotting the results
    plt.figure(figsize=(10, 6))
    plt.plot(levels, training_accuracies, label="Training Data Accuracy", marker="o")
    plt.plot(
        levels, evaluation_accuracies, label="Evaluation Data Accuracy", marker="o"
    )
