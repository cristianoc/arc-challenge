...
## Findings and retained candidate

Disposition and compact results: [experiment ledger](../RESULTS.md#003-objects--are-object-interpretations-expressively-and-predictively-useful).
Read the direct [full report](../evidence/003-objects/full/report.md) and the
[illustrated object example](../evidence/003-objects/object-example.png).
The preregistered candidate criterion is met; the stable solver remains unchanged.
Source `dc8ada2` contains the fixed grammar and initial runs; `c8b4552` adds
shared-fit reporting and clean zero formatting. Those reporting changes leave
all saved program pools identical. The illustrative plotting helper was added
after scoring; it performs no search and does not change the experiment.

Frozen pools: `out/experiments/003-objects/20260918T060901.136402Z-full/pools/`
(relative to `symarc/`). There are full-fit grid/object pools for all 400 tasks,
and depth-3 grid pools for the eight audited tasks. All carry original hashes
in the retained manifest; they can be regenerated by the runner.

### Remaining integration work

- Introduce a shared program interface that can evaluate either a grid sequence
  or an object AST. Move accepted object code into core; remove the experimental
  copy, preserving one implementation. Keep transformation definedness explicit.
- Check interaction with the existing closure/equivariance machinery, which
  currently accepts only grid-sequence programs. Do not assume pixel symmetry
  hypotheses remain helpful for object programs.
- Specify cross-language selection and complexity priors before integration.
  This experiment's reserved-example gate matches grid-first fallback; it does
  not justify integrating an entropy-based gate as an accuracy improvement.
- Verify complete-solver regression and new object cases. The seven gains are
  also absent from the retained complete-solver baseline, but production
  integration itself has not been benchmarked.
- For representation research, distinguish equivalent segmentation labels from
  distinct interpretations and examine robustness to the reserved example.
  One fixed validation pair and a narrow grammar are limited evidence. Object
  relations, nested structures and multistep actions remain outside this study.


The object grammar and inference functions are exposed in `src/lib.rs`; the CLI
imports that implementation. `../run.py` provides the shared serial harness,
with this directory's `run.py` preserving the original commands. The refactor
preserves all 48 pilot pools byte-for-byte. The completed interpretation-selection
study is recorded in [004's report](../evidence/004-competition/full/report.md);
it does not change this candidate's language or promote a new selection policy.
